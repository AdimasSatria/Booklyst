...
export default function BookDetailPage() {
  ...
  // Cari buku dari listedBooks
  const foundBook = listedBooks.find(b => b.id === bookId);

  // Simpan ke state lokal agar data tidak hilang saat status buku berubah setelah dibeli
  const [book, setBook] = useState(foundBook);
    ...
    <main className="min-h-screen bg-brand-base pt-20 pb-24">
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-start">
          {/* Left: Sticky Image (Hanya muncul jika belum success, atau disembunyikan saat sukses agar card tampil penuh di tengah) */}
          {step !== "success" && (
          ...
          {/* Right: Steps Flow (Jika success, lebarkan kolom agar card di tengah memenuhi ruang) */}
          <div className={`py-4 ${step === "success" ? "md:col-span-2 max-w-lg mx-auto w-full" : ""}`}>
            {step === "detail" && (
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                ...
                <div className="space-y-4 mb-8">
                  {[
                    ...
                    <label
                      key={method.id}
                      className={`flex items-start gap-4 p-5 rounded-2xl border-2 cursor-pointer transition-all ${paymentMethod === method.id ? "border-black bg-brand-surface" : "border-brand-secondary hover:border-brand-secondary"
                        }`}
                    >
                      <input
                        type="radio"
                ...
                {paymentMethod === "cash" ? (
                  ...
                  <div className="space-y-4 mb-8">
                    {[
                      ...
                      <label
                        key={loc}
                        className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${deliveryLocation === loc ? "border-black bg-brand-surface" : "border-brand-secondary hover:border-brand-secondary"
                          }`}
                      >
                        <input
                          type="radio"
            ...
            {step === "success" && (
              <motion.div
                ...
                className="bg-white p-8 md:p-10 rounded-3xl shadow-xl border border-neutral-100 text-center my-auto"
              >
                ...
                <h2 className="text-3xl font-bold mb-3 text-neutral-800">Pesanan Berhasil!</h2>
                <p className="text-neutral-500 mb-8 max-w-sm mx-auto leading-relaxed">
                  Penjual telah dihubungi. Silakan pantau status pesanan dan detail transaksi di Dashboard Anda.
                </p>
                <button
                  onClick={() => router.push("/profile")}
                  className="w-full py-4 bg-brand-primary text-brand-base text-lg font-bold rounded-2xl hover:bg-neutral-800 transition-colors shadow-lg"
                >
                  Lihat Dashboard
                </button>
              </motion.div>
