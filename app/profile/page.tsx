...
import { BookMarked, Wallet, Store, User, X, Clock, Plus, Package } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const handleUploadSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    uploadNewBook({
      ...
      imageUrl: formData.get("imageUrl") as string || "https://covers.openlibrary.org/b/isbn/9780132350884-L.jpg", // fallback
      seller: { name: user.name, prodi: user.major, angkatan: "2024", wa: "628000000000" }
    });
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === "orders" && (
              activeOrders.length === 0 ? renderEmpty("Tidak ada pesanan aktif.") : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {activeOrders.map((order) => (
                    <div key={order.id} className="bg-white rounded-3xl p-5 border border-neutral-200 flex flex-row items-center gap-4 sm:gap-6 shadow-sm">
                      ...
                      <div className="px-3 py-1.5 rounded-full bg-amber-50 text-amber-600 font-bold text-[10px] sm:text-xs shrink-0 text-center">
                        {order.status}
                      </div>
                    </div>
                  ))}
